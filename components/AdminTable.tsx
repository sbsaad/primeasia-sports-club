...
export default function AdminTable({ rows, initialDates }: Props) {
  ...
  const [startDate, setStartDate] = useState(toLocalDateTimeLocal(initialDates?.start));
  const [endDate, setEndDate] = useState(toLocalDateTimeLocal(initialDates?.end));
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      let auditReportText = [
        "===================================================",
        "Primeasia University Games and Sports Club Recruitment 2026",
  ...
  const buildDetailsText = (row: SubmissionRow): string => {
    return [
      ...
      "",
      "========================================",
      "Generated by Primeasia University Games and Sports Club Admin System",
    ].join("\n");
  };
